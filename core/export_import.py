...
class DataExporter:
    ...
    def __init__(self, user_id: int):
        ...
        # Создаём директорию только если пользователь существует
        if self.user is not None:
            self.export_dir = f"exports/user_{user_id}"
            os.makedirs(self.export_dir, exist_ok=True)
        else:
            # fallback: общая папка для неизвестного пользователя
            self.export_dir = "exports/unknown_user"
            os.makedirs(self.export_dir, exist_ok=True)
    ...
    def export_to_json(self, include_history: bool = True, filename: Optional[str] = None) -> str:
        ...
        data = {
            ...
            'user': {
                ...
                'age': self.user.age if self.user else 0,
                'weight': self.user.weight if self.user else 0,
                'height': self.user.height if self.user else 0,
                'goal': self.user.goal if self.user else '',
                'activity_level': self.user.activity_level if self.user else '',
                'target_calories': self.user.target_calories if self.user else 0,
                'level': self.user.level if self.user else 1,
                'xp': self.user.xp if self.user else 0,
                'streak_days': self.user.streak_days if self.user else 0
            },
            'meals': [],
    ...
    def export_to_pdf(self, report_type: str = 'weekly', filename: Optional[str] = None) -> str:
        ...
        user_goal = self._get_goal_name(self.user.goal) if self.user else ""
        user_weight = f"{self.user.weight} кг" if self.user else ""
        user_height = f"{self.user.height} см" if self.user else ""
        user_level = f"{self.user.level}" if self.user else ""
        user_streak = f"{self.user.streak_days}" if self.user else ""
        ...
        meals_table = Table(meals_data,
                            colWidths=[1 * inch, 2.5 * inch, 0.8 * inch, 0.6 * inch, 0.6 * inch, 0.6 * inch])
...
def test_export_import():
    ...
    # Получаем первого пользователя (например, с id=1)
    user_id = 1
